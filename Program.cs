...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            return shows.OrderBy(s => s.Name).Select(s => s.Name).ToList(); // need .Select() because it's looking for a string and not the entire Show
        }
        ...
        {
            return shows.Where(s => s.Genres.Contains("Crime")).Count(s => s.ImdbRating > 7.0);
        }
        ...
        {
            return string.Join(", ", shows.Select(s => s.Name).Take(shows.Count - 1)) + " and " + shows.LastOrDefault().Name;
        }
